import torch
from graph_models import GraphEBM, GraphFC, GraphPonder, GraphRecurrent#, IterativeFC
...
def test(train_dataloader, model, FLAGS, step=0, gen=False, test_call=False):
    ...
    model.train()
    if test_call and not(gen):
        return min(best_test_error, min_dist_energy.item())
    ...
    for epoch in range(FLAGS.num_epoch):
        for data in train_dataloader:
            ...
            if it % FLAGS.save_interval == 0 and rank_idx == 0:
                ...
                in_dist_loss = test(test_dataloader, model, FLAGS, step=it, gen=False)

                print("Generalization performance .......................")
                test(gen_dataloader, model, FLAGS, step=it, gen=True)

                if (in_dist_loss == best_test_error):
                    # print("best model")
